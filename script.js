    ...
    */
    ...
    async function fetchCurrentlyPlaying() {
        try {
            ...
            if (!response.ok) {
                throw new log(`HTTP error! status: ${response.status}`);
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function savePopupState(popup) {
        ...
        const prev = JSON.parse(localStorage.getItem(`popupState-${popupId}`)) || {};

        const isMinimized = popup.style.display === 'none';

        const state = {
            top: isMinimized ? prev.top : popup.offsetTop,
            left: isMinimized ? prev.left : popup.offsetLeft,
            width: isMinimized ? prev.width : popup.offsetWidth,
            height: isMinimized ? prev.height : popup.offsetHeight,
        ...
        newPopup.innerHTML = `
            <div class="popupHeader">
                ...
                <button style="border: 0px; background-color: transparent;" onclick="closePopup('${popupId}')">
                    <img height="20px" src="/images/Exit.webp">
                </button>
            </div>
    ...
    function addTaskbarItem(popupId, title) {
        ...
        const popup = document.getElementById(popupId);
        const iconEl = popup?.querySelector('.popupIcon');
        const iconSrc = iconEl?.getAttribute('src');
        taskbarItem.innerHTML = `
            <img src="${iconSrc}" alt="" class="taskbarIcon">
            <span>${title}</span>
        `;
...
document.addEventListener("DOMContentLoaded", function() {
    const logo = document.querySelector(".taskbarlogo.interests");
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                switch(theme) {
                    case 'red':
                        logo.src = '/images/windnowsxplogo2010red.webp';
                        break;
                    case 'pink':
                        logo.src = '/images/windnowsxplogo2010pink.webp';
                        break;
                    case 'green':
                        logo.src = '/images/windnowsxplogo2010green.webp';
                        break;
                    default:
                        logo.src = '/images/windnowsxplogo2010red.webp'; // default red bo jest pierwszy
                }
            }
        });
    }, {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (nickFromURL) {
        localStorage.setItem("chatNick", nickFromURL);
    ...
    if (chatNick && storedNick) {
        chatNick.value = storedNick;
    }
